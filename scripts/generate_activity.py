...
def render_svg(weeks, total, p):
    ...
    add('<text x="54" y="64" font-size="23" font-weight="700" letter-spacing="5" '
        'fill="%s">BUILD ACTIVITY</text>' % p["mid"])
    add('<text x="1146" y="64" text-anchor="end" font-size="19.5" font-weight="700" '
    ...
    for val, y in ((mx, Y_TOP), (mx // 2, (Y_TOP + Y_BASE) // 2), (0, Y_BASE)):
        ...
        add('<text x="%d" y="%d" text-anchor="end" font-size="16" fill="%s">%d</text>'
            % (X0 - 8, y + 6, p["dim"], val))
    ...
    for i, (dstr, v) in enumerate(weeks):
        ...
        if month != prev_month:
            if prev_month is not None:
                add('<text x="%.1f" y="%d" font-size="16.5" letter-spacing="2" '
